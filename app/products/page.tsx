    ...
    if (params.min || params.max) {
      ...
      products = products.filter((product) =>
        product.variants.some(
          (variant) =>
            Number(variant.price || 0) >= min &&
            Number(variant.price || 0) <= max
        )
      );
    }
  ...
  return (
    <div className="min-h-screen py-12 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-10">
          <p className="text-jungle-400 text-sm font-semibold uppercase tracking-widest mb-2">
            Herbal Communities
          </p>
          ...
          <div className="flex-1 min-w-0">
            {products.length === 0 ? (
              <div className="glass rounded-2xl p-16 text-center">
                ...
                <a
                  href="/products"
                  className="inline-block mt-4 text-jungle-300 hover:text-white transition text-sm font-semibold"
                >
                  Clear filters →
                </a>
              </div>
